<?php
	...
	function crear_sql_busqueda($bd)
	{
		if(isset($_POST["sel_opcion"]) and $_POST["sel_opcion"]=="fecha")
		{
			...
			$sql="select ae.id_abono_empleado, ae.fecha, concat(e.nombre,' ',e.apellido) empleado, case when ae.efectivo = 1 then aee.monto else '' end as 'efectivo', case when ae.transferencia = 1 then aet.monto else '' end as 'transferencia', case when ae.transferencia = 1 then aet.referencia else '' end as 'referencia', ifnull(aee.monto,0) + ifnull(aet.monto,0) total from abono_empleado ae inner join empleado e on ae.empleado_telf = e.empleado_telf left join abono_empleado_efectivo aee on ae.id_abono_empleado = aee.id_abono_empleado left join abono_empleado_transferencia aet on ae.id_abono_empleado = aet.id_abono_empleado where ".$where." order by ae.fecha_num asc;";
			unset($where);
		}
		elseif(isset($_POST["sel_opcion"]) and $_POST["sel_opcion"]=="todo")
		{
			$sql="select ae.id_abono_empleado, ae.fecha, concat(e.nombre,' ',e.apellido) empleado, case when ae.efectivo = 1 then aee.monto else '' end as 'efectivo', case when ae.transferencia = 1 then aet.monto else '' end as 'transferencia', case when ae.transferencia = 1 then aet.referencia else '' end as 'referencia', ifnull(aee.monto,0) + ifnull(aet.monto,0) total from abono_empleado ae inner join empleado e on ae.empleado_telf = e.empleado_telf left join abono_empleado_efectivo aee on ae.id_abono_empleado = aee.id_abono_empleado left join abono_empleado_transferencia aet on ae.id_abono_empleado = aet.id_abono_empleado order by ae.fecha_num asc;";
		}
